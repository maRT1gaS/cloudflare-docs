...
To use the `qr-image` package, configure the `type` to `"webpack"`. This instructs Wrangler to use [Webpack](/workers/cli-wrangler/webpack/) to package your project for deployment. (Learn more about [`type` configuration](/workers/cli-wrangler/configuration/).)
