...
Use only the fields [`http.request.uri.*`](/firewall/cf-firewall-language/fields#uri-argument-and-value-fields), [`http.request.headers.*`](/firewall/cf-firewall-language/fields#http-header-fields), and [`http.request.accepted_languages`](/firewall/cf-firewall-language/fields#field-http-request-accepted_languages) in rewrite expressions.

The `concat()` and `regex_replace()` functions can appear only **once** in a rewrite expression. For more information on these functions, refer to [Transformation functions](/firewall/cf-firewall-language/functions#transformation-functions).
{{</Aside>}}
