...
A URL Rewrite Rule can perform a **static rewrite** or a **dynamic rewrite**. Static rewrites replace a given part of a request URL (path or query string) with a static string. Dynamic rewrites support more advanced scenarios where you use an [expression](/firewall/cf-firewall-rules/fields-and-expressions) to define the resulting path or query string.

To create a URL Rewrite Rule in the dashboard, refer to [Create a URL Rewrite Rule in the dashboard](/rules/transform/url-rewrite/create-dashboard/).

To create a URL Rewrite Rule via API, refer to [Create a URL Rewrite Rule via API](/rules/transform/url-rewrite/create-api/).
