...
- Define the [URL rewrite parameters](/rules/transform/url-rewrite/parameters/) in the `action_parameters` field according to the type of URL rewrite (static or dynamic)
...
The following example sets the rules of an existing phase ruleset (`{ruleset-id}`) to a single URL Rewrite Rule — performing a static rewrite of the URI path — using the [Update ruleset](/ruleset-engine/rulesets-api/update) method:
...
The following example sets the rules of an existing phase ruleset (`{ruleset-id}`) to a single URL Rewrite Rule — performing a dynamic rewrite of the URI path — using the [Update ruleset](/ruleset-engine/rulesets-api/update) method:
