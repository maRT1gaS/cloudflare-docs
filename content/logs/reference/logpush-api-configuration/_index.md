...
The table below summarizes the job operations available. All the examples in this page are for zone-scoped data sets. Account-scoped data sets should use `/accounts/<ACCOUNT_ID>` instead of `/zone/<ZONE_ID>`. For more information, refer to the [Log fields](/logs/reference/log-fields/) page.
...
| --------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| POST      | Create job                                   | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/jobs</em>                                    |
| GET       | Retrieve job                                 | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/jobs/\<job></em>                        |
| GET       | Retrieve all jobs for all data sets          | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/jobs</em>                                    |
| GET       | Retrieve all jobs for a data set             | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/datasets/\<dataset>/jobs</em>           |
| GET       | Retrieve all available fields for a data set | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/datasets/\<dataset>/fields</em>         |
| GET       | Retrieve all default fields for a data set   | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/datasets/\<dataset>/fields/default</em> |
| PUT       | Update job                                   | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/jobs/\<job></em>                        |
| DELETE    | Delete job                                   | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/jobs/\<job></em>                        |
| POST      | Check whether destination exists             | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/validate/destination/exists</em>             |
| POST      | Get ownership challenge                      | <em>https://api.cloudflare.com/client/v4/zones/\<zone\_id>/logpush/ownership</em>                               |
...
In Logpush, the *Logpull options* would be: `"logpull_options": "fields=RayID,EdgeStartTimestamp"`. *See [Logpull API parameters](/logs/logpull/requesting-logs/#parameters)* for more info.
