...
Follow [these instructions](/argo-tunnel/downloads/) to download and install `cloudflared` on the machine hosting the resource.
...
Next, protect the subdomain you plan to register with a Cloudflare Access policy. Follow [these instructions](/cloudflare-one/setting-up-access/configuring-access-policies/) to build a new policy to control who can connect to the resource.
