...
1.  Do one of the following actions:
    ...
    - On the pool, set the [origin weight](/load-balancing/understand-basics/weighted-load-balancing/) to `0` (though traffic may still reach the origin if it is included in multiple pools).
2.  Monitor [Load Balancing Analytics](/load-balancing/load-balancing-analytics/) to make sure no requests are reaching the pool.
