...
Notice that you can specify the request time period using a data set filter (see [*Filtering*](/analytics/graphql-api/features/filtering/)). The `continuous` parameter is no longer needed because GraphQL Analytics is designed to provide data as soon as it is available.
